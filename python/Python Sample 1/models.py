...
class Employee(AccountEmployee):
    ...
    def prepare_attendance(self, dates):
        ...
        for attendance in self.monthly_attendance:
            ...
            if attendance.date.weekday() in [5, 6] \
                    and attendance.day_status[0] in [Attendance.LOSS_OF_PAY, Attendance.LOP_HALF_DAY]:
                sandwich_days += 1
            elif attendance.date.weekday() not in [5, 6] and attendance.day_status[0] != 'H':
                self._credited_days += attendance.day_status[1]
        ...
        self._credited_days -= sandwich_days
    ...
    def _day_status(self, ignore_leave_status=True):
        ...
        if self.leave_type1 and is_valid_status:
            ...
            if self.date <= today:
                ...
                if self.leave_type2:
                    leave_day_value += self.leave_type2_value
        ...
        if leave_day_value < 1:
            ...
            if self.date <= today:
                if status == Attendance.UNCERTAIN:
                    ...
                    if self.duration:
                        status_by_duration, day_value_by_duration = self._status_by_duration()
                        if day_value_by_duration:
                            day_value = day_value_by_duration
                    ...
                    if duration < 3:
                        ...
                        # Use case: if user has applied EH and added some logs
                        # so day value should be affected by the values of the timelog added
                        status_by_duration, day_value_by_duration = self._status_by_duration()
                        day_value += day_value_by_duration
    ...
    def _status_by_duration(self):
        ...
        if self.duration:
            if self.duration >= 3 and self.duration < 6:
                ...
                status_by_duration = Attendance.PRESENT
                day_value = 1
        return status_by_duration, day_value
...
class AppliedLeaves(models.Model):
    ...
    def can_approve(self, by, raise_exception=False):
        if self.status not in [LEAVE_APPROVAL_PENDING]:
            if raise_exception:
                raise ValidationError(_('LEAVE_APPROVE_ERR'))
            return False
        ...
        if not by.has_perm('attendance.can_manage_full_attendance_leaves'):
            if by not in self.employee.get_supervisors():
                if raise_exception:
                    raise ValidationError(_('LEAVE_APPROVE_ADMIN_PERMISSON_ERR'))
                return False
    ...
    def can_decline(self, by, raise_exception=False):
        if self.status not in [LEAVE_APPROVAL_PENDING]:
            if raise_exception:
                raise ValidationError(_('LEAVE_DECLINE_ERR'))
            return False
    ...
    def can_cancel(self, by, raise_exception=False):
        if self.employee == by.employee:
            ...
            if not by.has_perm('attendance.can_manage_full_attendance_leaves'):
                if by not in self.employee.get_supervisors():
                    if raise_exception:
                        raise ValidationError(_('LEAVE_CANCEL_ADMIN_PERMISSON_ERR'))
                    return False
    ...
    def _cancel(self, by, reason, new_status=LEAVE_CANCELED):
        if new_status not in [LEAVE_CANCELED, LEAVE_DECLINED]:
            raise ValidationError(_('WORNG_CURRENT_STATUS_ON_CALNCEL'))
